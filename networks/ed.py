...
class ED_model(nn.Module):
    ...
    def forward(self, input):
        ...
        out_seg = self.decoder(features, 0)
        out_map_tmax = self.map_out_activation(self.decoder(features, 1))
        out_map_mtt = self.map_out_activation(self.decoder(features, 2))
        out_map_cbv = self.map_out_activation(self.decoder(features, 3))
        out_map_cbf = self.map_out_activation(self.decoder(features, 4))
