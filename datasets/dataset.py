...
class CTPDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            seg = np.zeros_like(data[:,:,0])  # No seg label, Empty dummy label
...
class RandomCrop(object):
    ...
    def cut(self, data, tmax, mtt, cbv, cbf, seg, sizes):
        ...
        data = data[h1:h1 + self.output_size[1], w1:w1 + self.output_size[0], d1:d1 + self.output_size[2]]
    ...
    def __call__(self, sample):
        data, tmax, mtt, cbv, cbf, seg = sample['data'], sample['tmax'], sample['mtt'], sample['cbv'], sample['cbf'], sample['seg']
        ...
        while True:
            ...
            if legion_centered_patch:
                ...
                y_min, y_max, x_min, x_max = np.min(cont[:,0]), np.max(cont[:,0]), np.min(cont[:,1]), np.max(cont[:,1])

                if self.offset_magnitude_ratio > 0:
                    x_offset = random.randint(-self.output_size[0] // self.offset_magnitude_ratio ,self.output_size[0] // self.offset_magnitude_ratio )
                    y_offset = random.randint(-self.output_size[1] // self.offset_magnitude_ratio ,self.output_size[1] // self.offset_magnitude_ratio )
                else:
                    x_offset = 0
                    y_offset = 0
                patch_center = ((x_max-x_min)// 2 + x_min + x_offset, (y_max-y_min)// 2 + y_min + y_offset)
            ...
            data_cut, tmax_cut, mtt_cut, cbv_cut, cbf_cut, seg_cut = self.cut(data, tmax, mtt, cbv, cbf, seg, (w1,h1,d1))
